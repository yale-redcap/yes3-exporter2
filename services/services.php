...
function update_export_table_settings()
{
    ...
    return $module->stdReturnObj( $result, $message, $updates, true );
}
...
function saveExportSpecification()
{
    ...
    // special handling for deprecated repeating layout
    if ( $qParams['export_layout'] === "r" && isset($_POST['convert_to_vertical']) && $_POST['convert_to_vertical'] == 1 ){

        $qParams['export_layout'] = "v"; // convert to vertical
        $module->sysmsg = "Note: the export layout 'repeating' has been deprecated and will be converted to 'vertical'.";
    }
    ...
    if ( $log_id ){
        ...
        $module->setExportCalculatedFields($qParams['export_uuid'], $log_id);
...
function getExportSpecification()
{
    ...
    if ( !$permission_export && !$permission_design ){
        ...
        $specification['column_count'] = $module->getExportColumnCount($specification);
    }

    $specification['sysmsg'] = $module->sysmsg;
    $specification['errmsg'] = $module->errmsg;
    $specification['export_code_filename_base'] = $module->exportFilenameBase($specification['export_name']);
...
function getExportSpecificationList():string
{
    ...
    $get_removed = (int) ($_POST['get_removed'] ?? 0);
    ...
    foreach($UUIDs as $u){
        ...
        $module->confirmSpecificationPermissions($s); // sets permission_export, error_messages, warning_messages
        ...
        $data[] = [
            ...
            'error_messages' => $module->escape($s['error_messages']),
            'warning_messages' => $module->escape($s['warning_messages']),
            'removed' => $s['removed'],
            'export_order' => ( $s['export_order'] ) ? $s['export_order'] : "0",
            'export_batch' => $s['export_batch'] ?? "0",
            'export_sascode' => $s['export_sascode'] ?? "0",
...
function getExportSpecificationList_beta(){
...
function getExportSpecificationList2( $get_removed=0 )
{
    ...
    foreach($UUIDs as $u){
        ...
        // retrieve or calculate the column count, typed as integer
        $column_count = $module->getExportColumnCount($s);
...
function downloadExportLog()
{
    ...
    foreach ( $module->recordGenerator($sql, [ $module->getProjectId(), EMLOG_TYPE_EXPORT_LOG_ENTRY, $export_uuid ]) as $x ){
        ...
        if ( !$bytes ) {
            ...
            $bytes = Yes3Fn::fputcsv($h, array_keys($x));

            $export_name = $x['export_name'];
        }

        $bytes += Yes3Fn::fputcsv($h, array_values($x));
    }
...
function getExportLogData()
{
    ...
    $subSql = "
SELECT x.*, p2.`value` AS `export_uuid`, p3.`value` AS `destination`, p4.`value` AS `user_name`
FROM redcap_external_modules_log x
  INNER JOIN redcap_external_modules_log_parameters p1 ON p1.log_id=x.log_id AND p1.name='log_entry_type'
  INNER JOIN redcap_external_modules_log_parameters p2 ON p2.log_id=x.log_id AND p2.name='export_uuid'
...
function downloadDmsp()
{
    ...
    return $module->downloadZip($export_uuid, true);
}
 ...
 * 
 * function: Yale\Yes3Exporter\sanitizeUploadSpec
 * 
 * @param mixed $uSpec
 * 
 * @return mixed
...
function get_wayback_html()
{
    ...
    foreach ( $specification_history as $x) {
        ...
        $user = $module->escape($x['export_username']);
...
function save_field_mappings()
{
    ...
    if ( $logId ){
        ...
        return "Success: YES3 Exporter Field Mappings for " . $module->escape($export_uuid) . " saved.";
    }
...
function get_project_settings():string 
{
    ...
    $reduced = $_POST['reduced'] ?? 0;

    if ( $reduced ){

        return get_reduced_project_settings();
    }
...
function get_reduced_project_settings() {
    global $module;
...
function get_specification_settings()
{
    ...
    for ( $i=0; $i<count($specification_keys); $i++ ){
        $x[] = [
            'specification_key' => $module->escape($specification_keys[$i]),
            'specification_name' => $module->escape($specification_names[$i]),
            'specification_description' => $module->escape($specification_descriptions[$i])
        ];
    }
...
function get_event_abbreviation_settings()
{
    ...
    for ( $i=0; $i<count($event_ids); $i++ ){
        $x[] = [
            'event_id' => $module->escape($event_ids[$i]),
            'event_abbreviation' => $module->escape($event_abbreviations[$i])
        ];
    }
 ...
 * 
 * function: Yale\Yes3Exporter\get_event_metadata
 * 
 * 
 * @return array
 * @throws Exception
...
ORDER BY e.day_offset
    ...
    foreach ($ee as $e){
        $project_event_metadata[] = [
            ...
            'event_label' => $module->escape($e['event_label'])
        ];
    }
 ...
 * 
 * function: Yale\Yes3Exporter\get_form_metadata_structures
 * 
 * 
 * @return array
 * @throws Exception
